...
async function validateApprovedCountAndMarks(client, application_type, application_id, parameter_id, approved_count, approved_marks) {
  try {
    const { table, jsonColumn, idField } = getTableMeta(application_type);

    const fds = await fetchApplicationFds(client, table, jsonColumn, idField, application_id);
    ...
    const numbers = coerceNumbers(approved_count, approved_marks, parameter);

    const countErr = validateCount(numbers, approved_count);
    if (countErr) return countErr;

    const marksErr = validateMarks(numbers, approved_marks);
    if (marksErr) return marksErr;

    const consistencyErr = validateConsistency(numbers, approved_count, approved_marks);
    if (consistencyErr) return consistencyErr;

    const boundsErr = validateBounds(numbers, approved_count, approved_marks);
...
function getTableMeta(application_type) {
  const isCitation = application_type === "citation";
  return {
    table: isCitation ? "Citation_tab" : "Appre_tab",
    jsonColumn: isCitation ? "citation_fds" : "appre_fds",
    idField: isCitation ? "citation_id" : "appreciation_id",
  };
}

async function fetchApplicationFds(client, table, jsonColumn, idField, application_id) {
...
function coerceNumbers(approved_count, approved_marks, parameter) {
  return {
    approvedCountNum: Number(approved_count) || 0,
    approvedMarksNum: Number(approved_marks) || 0,
    originalCount: Number(parameter.count) || 0,
    maxMarks: Number(parameter.max_marks) || 0,
    perUnitMark: Number(parameter.per_unit_mark) || 0,
  };
}

function validateCount({ approvedCountNum }, approved_count) {
  const invalid = approved_count !== undefined && (isNaN(approvedCountNum) || approvedCountNum < 0);
  return invalid ? { isValid: false, message: "Approved count must be a valid non-negative number" } : null;
}

function validateMarks({ approvedMarksNum }, approved_marks) {
  const invalid = approved_marks !== undefined && (isNaN(approvedMarksNum) || approvedMarksNum < 0);
  return invalid ? { isValid: false, message: "Approved marks must be a valid non-negative number" } : null;
}

function validateConsistency({ approvedCountNum, approvedMarksNum, perUnitMark, maxMarks }, approved_count, approved_marks) {
...
function validateBounds({ approvedCountNum, approvedMarksNum, originalCount, maxMarks }, approved_count, approved_marks) {
