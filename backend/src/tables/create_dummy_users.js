...
async function createDummyUsers() {
  ...
  try {
    ...
    // Get role IDs
    const roleQuery = await client.query('SELECT role_id, role_name FROM Role_Master');
    const roles = {};
    ...
    for (const user of users) {
      try {
        ...
        if (!roleId) {
          console.log(`❌ Role ${user.user_role} not found, skipping ${user.name}`);
          continue;
        }
    ...
    const brigadeCount = await client.query('SELECT COUNT(*) FROM Brigade_Master');
    ...
    const commandCount = await client.query('SELECT COUNT(*) FROM Command_Master');
    console.log(`🏛️ Command entries: ${commandCount.rows[0].count}`);
    
    const divisionCount = await client.query('SELECT COUNT(*) FROM Division_Master');
