...
const AllApplicationsList = () => {
  ...
  const [meta, setMeta] = useState<AllApplicationsResponse['meta'] | null>(null);
  ...
  const fetchAllApplications = async () => {
    try {
      ...
      const response = await Axios.get<AllApplicationsResponse>(
        `/api/applications/all-app-list?${queryParams.toString()}`
      );
        ...
        <table className="table-style-2 w-100">
          ...
          <tbody>
            {loading ? (
              ...
              applications.map((application: AllApplication) => (
