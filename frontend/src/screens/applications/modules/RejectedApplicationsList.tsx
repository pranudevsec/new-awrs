...
interface RejectedApplicationsResponse {
  success: boolean;
...
const RejectedApplicationsList = () => {
  ...
  const fetchRejectedApplications = async () => {
    try {
      ...
      const response = await Axios.get<RejectedApplicationsResponse>(
        `/api/applications/all-app-reject?${queryParams.toString()}`
      );
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message ?? "Failed to fetch rejected applications");
      }
    } catch (error: any) {
      toast.error(
        error.response?.data?.message ?? "Error fetching rejected applications"
      );
    } finally {
      setLoading(false);
    }
        ...
        <table className="table-style-2 w-100">
          ...
          <tbody>
            {loading ? (
              ...
              applications.map((application: RejectedApplication) => (
