...
const Dashboard = () => {
  ...
  const { loading, dashboardStats } = useAppSelector(
    (state) => state.commandPanel
  );
  const { units: hqApplications, loading: appLoading } = useAppSelector(
  ...
  const totalMarksDomain = getDynamicDomain(
    hqChartData.map((u) => u.totalMarks)
  );

  // Show loader
  if (loading || appLoading) return <Loader />;
  const dummyHqChartData = [
    { name: "Unit A", totalMarks: 85 },
    { name: "Unit B", totalMarks: 72 },
    { name: "Unit C", totalMarks: 95 },
    { name: "Unit D", totalMarks: 60 },
    { name: "Unit E", totalMarks: 78 },
    { name: "Unit F", totalMarks: 88 },
  ];
  return (
    <div className="dashboard-section">
      ...
      <div className="row mb-4 row-gap-4">
        <div className="col-lg-4">
          <UnitScoreChart
            data={dummyHqChartData}
            dataKey="totalMarks"
            title="Top Units by Total Score"
