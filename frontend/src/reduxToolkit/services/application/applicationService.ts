...
import type { FetchApplicationUnitDetailResponse, FetchApplicationUnitsResponse } from './applicationInterface';
...
>('applications/fetchUnitDetail', async (params, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || 'Failed to fetch unit detail');
  }
});
...
export const fetchSubordinates = createAsyncThunk<
  ...
  FetchUnitsParams | undefined
>(
  'applications/fetchSubordinates',
  async (params, { rejectWithValue }) => {
    try {
      const queryParams = new URLSearchParams();
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'Failed to fetch subordinate applications');
        return rejectWithValue(response.data.message);
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Error fetching subordinate applications');
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch subordinate applications');
    }
  }
);
